...
def eva02_attention_forward(
    ...
    return x
import timm
def replace_eva02_attn_with_flash_attn():
    cuda_major, cuda_minor = torch.cuda.get_device_capability()
    if cuda_major < 8:
        logging.warning(
            "Flash attention is only supported on A100 or H100 GPU during training due to head dim > 64 backward."
    ...
    timm.models.eva.EvaAttention.forward = eva02_attention_forward
    
    


replace_llama_attn_with_flash_attn()
replace_eva02_attn_with_flash_attn()
from pipeline.train.pretraining_cc3m import main
